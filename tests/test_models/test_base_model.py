...
class TestBase(unittest.TestCase):
    ...
    def test_save(self):
        """
            Checks that the dates differ in the
    ...
    def test_type_created_at(self):
        """
            Test that the new_model updated_at
    ...
    def test_type_updated_at(self):
        """
            Test that the new_model created_at
            data type is datetime.
        """
        my_model_dict = self.my_model.to_dict()
        new_model = BaseModel(my_model_dict)
    ...
    def test_compare_dict(self):
        """
            Test that the new_model and my_model
        ...
        """ Compare timestamps without microsecond precision"""
        self.assertEqual(
            my_model_dict['created_at'][:-7], new_model_dict['created_at'][:-7],
            f"created_at mismatch: {my_model_dict['created_at']} != {new_model_dict['created_at']}"
        )
        self.assertEqual(
            my_model_dict['updated_at'][:-7], new_model_dict['updated_at'][:-7],
            f"updated_at mismatch: {my_model_dict['updated_at']} != {new_model_dict['updated_at']}"
        )
